...
main <- function(){
  ...
  # make histogram of age distribution colored by sex and cardiovascular health category. add median lines
  sex_median_values_age <- meta |>
    group_by(sex) |>
    summarize(median_age = median(age_years, na.rm = TRUE))
  
  cv_median_values_age <- meta |>
    group_by(Cardiometabolic_status) |>
    summarize(median_age = median(age_years, na.rm = TRUE))
  ...
  p_age_cv <- ggplot(meta, aes(x = age_years, fill = Cardiometabolic_status, color = Cardiometabolic_status)) +
    geom_histogram(alpha = 0.6) +
    labs(
      x = "Age (years)",
      y = "Number of people",
  ...
  cowplot::save_plot(plot = comb_hists, "results/01-fiber_met_age_hists.png", base_height = 6, base_width = 12)
